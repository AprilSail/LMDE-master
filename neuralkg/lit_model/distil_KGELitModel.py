...
import plm_score.plm_main as LMmain
...
class distil_KGELitModel(BaseLitModel):
    ...
    def __init__(self, model, args=None, model_tea=None, lm_model_tea=None):
        super().__init__(model, args, )
        self.model = model
        self.lmlmlm = LMmain.LM_trainer
        self.model_tea = model_tea
        self.lm_model_tea = lm_model_tea
        self.args = args
    ...
    def training_step(self, batch, batch_idx):
        ...
        # tail_predict_ranks, head_predict_ranks = link_predict(batch, self.model, prediction='all')

        # 计算teacher的score
        pos_score_tea = self.model_tea(pos_sample)
        neg_score_tea = self.model_tea(pos_sample, neg_sample, mode)

        score_of_lm = self.lmlmlm.get_embedding_from_LM(embedding_data_list=pos_sample)
        ...
        if self.args.use_weight:
            ...
            loss = self.loss(pos_score, neg_score, pos_score_tea, neg_score_tea, score_of_lm, subsampling_weight)
    ...
    def configure_optimizers(self):
        ...
        """
        milestones = int(self.args.num_epoch / 2)
