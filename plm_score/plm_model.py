...
class LMKE(nn.Module):
    def __init__(self, lm_model, n_ent, n_rel, add_tokens, contrastive):
        ...
        self.n_ent = n_ent
        self.n_rel = n_rel
        self.hidden_size = lm_model.config.hidden_size  # hidden_size = 768
    ...
    def forward_transe(self, positions, mode):
        ...
        if mode in ['link_prediction_h', 'head']:
            triple_score = self.score_triples_transe(
                can_ent_emb.expand(batch_size, can_ent_emb.shape[0], can_ent_emb.shape[1]), r_embs.unsqueeze(1),
                t_embs.unsqueeze(1))
        elif mode in ['link_prediction_t', 'tail']:
            triple_score = self.score_triples_transe(h_embs.unsqueeze(1), r_embs.unsqueeze(1),
                                                     can_ent_emb.expand(batch_size, can_ent_emb.shape[0],
                                                                        can_ent_emb.shape[1]))

        return triple_score

    def my_forward_transe(self, positions):
        batch_size = len(positions)
        device = self.lm_model_given.device
        ...
        triple_score = self.score_triples_transe(h_embs.unsqueeze(1), r_embs.unsqueeze(1), t_embs.unsqueeze(1))
    ...
    def forward_rotate(self, positions, mode):
        ...
        if mode in ['link_prediction_h', 'head']:
            triple_score = self.score_triples_rotate(
                can_ent_emb.expand(batch_size, can_ent_emb.shape[0], can_ent_emb.shape[1], can_ent_emb.shape[2]), \
                r_embs.unsqueeze(1), t_embs.unsqueeze(1), mode)
        elif mode in ['link_prediction_t', 'tail']:
            triple_score = self.score_triples_rotate(h_embs.unsqueeze(1), r_embs.unsqueeze(1), \
                                                     can_ent_emb.expand(batch_size, can_ent_emb.shape[0],
                                                                        can_ent_emb.shape[1], can_ent_emb.shape[2]),
                                                     mode)
