...
class Trainer:
    def __init__(self, data_loader, model, tokenizer, optimizer, scheduler, device, hyperparams):
        ...
        # get LM embedding 的预处理（加速代码执行）
        self.model_em = model

        n_ent = self.model_em.n_ent
        n_rel = self.model_em.n_rel

        ent2id = self.data_loader.ent2id
        rel2id = self.data_loader.rel2id
        self.entity_list = self.data_loader.entity_list
        self.model_em.eval()

        self.sigmoid = torch.nn.Sigmoid()
        self.dl_statistics = self.data_loader.statistics
        # max_degree = dl_statistics['max_degree']
        self.degree_group = self.dl_statistics['degree_group']
        # count_degree_group = dl_statistics['count_degree_group']
        self.degrees = self.dl_statistics['degrees']

        if self.hyperparams['contrastive']:
            # calc target embeddings
            batch_size = 128
            self.ent_target_encoded = torch.zeros((n_ent, self.model_em.hidden_size)).to(self.device)
            rel_target_encoded = torch.zeros((n_rel, self.model_em.hidden_size)).to(self.device)
            ...
            with torch.no_grad():
                ...
                for batch in batch_list:
                    batch_targets = [self.entity_list[_] for _ in batch]
                    target_inputs, target_positions = self.data_loader.batch_tokenize_target(targets=batch_targets)
                    target_inputs.to(self.device)
                    target_encodes = self.model_em.encode_target(target_inputs, target_positions,
                                                                 mode='link_prediction_h')
                    self.ent_target_encoded[batch] = target_encodes

        entity_path = r"./dataset/WN18RR/entities.dict"
        self.entity_dic = {}
        with open(entity_path, 'r', encoding='utf8') as fil:
            for line in fil.readlines():
                a, b = line.strip('\n').split('\t')
                self.entity_dic[str(a)] = b

        rel_path = r"./dataset/WN18RR/relations.dict"
        self.rel_dic = {}
        with open(rel_path, 'r', encoding='utf8') as fil:
            for line in fil.readlines():
                c, d = line.strip('\n').split('\t')
                self.rel_dic[str(c)] = d
        ...
        if hyperparams['contrastive']:
            ...
            with torch.no_grad():
                ...
                for batch in batch_list:
                    ...
                    print(ent_target_encoded)
                    print(ent_target_encoded.shape)

        f = open(self.result_log, 'a')
        # -max_desc_length=512.txt' mode='a' encoding='cp936'>
        print('Load Path: {} split: {}'.format(self.load_path, split))
                ...
                for given in tqdm(groundtruth[split][target].keys()):
                    ...
                    if not hyperparams['no_use_lm']:
                        if hyperparams['contrastive']:
                            ...
                            preds = model.match(target_preds, target_encodes, triple_degrees, mode, test=True,
                                                ent_list_degrees=ent_list_degrees)
                        else:
    ...
    def get_embedding_from_LM(self, split='train', embedding_data_list=None):  # 模仿link_prediction函数
        # print("Get_embedding_from_LargeModel...")
        id_to_name = []
        for i in range(len(embedding_data_list)):
            h = int(embedding_data_list[i][0])
            r = int(embedding_data_list[i][1])
            t = int(embedding_data_list[i][2])
            id_to_name.append((self.entity_dic[str(h)], self.rel_dic[str(r)], self.entity_dic[str(t)]))

        part_groundtruth = id_to_name

        with torch.no_grad():
            scores_lm = []
            for given in part_groundtruth:
                triples = [(given[0], given[1], given[2])]

                _, positions = self.data_loader.batch_tokenize(triples, mode='triple_classification')

                triple_score = self.model_em.my_forward_transe(positions).squeeze()
                scores_lm.append([float(triple_score)])

        return torch.tensor(scores_lm, device='cuda:0')
